...
const consultaApipadrao = () => {
    fetch(`https://api.hgbrasil.com/weather?format=json-cors&key=3ce1acab&city_name=recife&array_limit=7&fields=only_results,city_name,city,date,temp,condition_slug,description,forecast,weekday,condition,max,min`)
            .then(response => {
...
const consultaApigeolocation = (latitude, longitude) =>{
    fetch(`https://api.hgbrasil.com/weather?format=json-cors&key=3ce1acab&lat=${latitude}&lon=${longitude}&array_limit=7&fields=only_results,city_name,city,date,temp,condition_slug,description,forecast,weekday,condition,max,min`)
    .then(response => {
        response.json().then(dados =>{
            criarClima(dados);
            criarCards(dados);
